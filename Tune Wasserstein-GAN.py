...
class WassersteinDiscriminator(nn.Module):
    def __init__(self, input_size, n_layers, n_channel, kernel_size, dropout):
        super().__init__()
        self.input_size = input_size  # Store the input size for dynamic adjustment
        self.tcn = TCN(input_size=input_size, output_size=1, num_channels=[n_channel] * n_layers,
                       kernel_size=kernel_size, dropout=dropout)

        # Adjust the input channels for the first convolution layer
        self.conv1 = weight_norm(nn.Conv1d(self.input_size, n_channel, kernel_size,
                                           stride=1, padding=(kernel_size - 1), dilation=1))

        # Additional linear layers with ReLU activation
        self.linear = nn.Sequential(
            nn.Linear(n_channel, n_channel),  # Adjusted input size to match num_channels
            nn.ReLU(),
            nn.Linear(n_channel, 1)
        )

    def forward(self, x, channel_last=True):
        # Apply the first convolution layer dynamically based on the input size
        x = self.conv1(x.view(-1, self.input_size, 1))

        # Continue with TCN
        output = self.tcn(x, channel_last)
...
def calculate_gradient_penalty(discriminator, real_data, fake_data, device):
...
def pretrain_discriminator_real(discriminator, optimizer, real_data, criterion, device):
    ...
    output_real = torch.sigmoid(discriminator(real_data)).view(-1, 1)

    # Flatten the tensors for binary cross-entropy loss
    output_real_flat = output_real.view(-1, 1)
    real_labels_flat = real_labels.view(-1, 1)

    loss_real = criterion(output_real_flat, real_labels_flat.float())
    loss_real.backward()
...
def normal_dist(mean, std_dev, minimum, maximum):
    return lambda current_value: clamp(
        current_value + random.gauss(mean, std_dev) * current_value, minimum, maximum)


# Define your parameter distributions
number_of_batches_dist = normal_dist(0, 0.5, 8, 64)
input_length_dist = normal_dist(0, 0.5, 50, 200)
batch_length_dist = normal_dist(0, 0.5, 50, 400)
step_size_dist = normal_dist(0, 0.5, 10, 100)
pretrain_epochs_dist = normal_dist(0, 0.5, 0, 10)
lr_gen_dist = normal_dist(0, 0.5, 0.00001, 0.0002)
lr_dis_dist = normal_dist(0, 0.5, 0.00001, 0.0002)
layers_gen_dist = normal_dist(0, 1.5, 1, 5)
layers_dis_dist = normal_dist(0, 1.5, 1, 5)
dim_gen_dist = normal_dist(0, 0.5, 50, 300)
dropout_dist = normal_dist(0, 0.5, 0.0, 0.4)
...
def train_GAN(number_of_batches, input_length, batch_length, step_size, pretrain_epochs, lr_gen, lr_dis, layers_gen,
    ...
    numpy_buffer.seek(0)
    loaded_data = np.load(numpy_buffer)
    dataset = torch.from_numpy(loaded_data).float()  # Convert to PyTorch tensor

    ngpu = 1
    device = torch.device("cuda:0" if (torch.cuda.is_available() and ngpu > 0) else "cpu")

    netD = WassersteinDiscriminator(input_size=1, n_layers=layers_dis, n_channel=1, kernel_size=8, dropout=dropout).to(
        device)
    ...
    for epoch in range(epochs):
        ...
        for i in range(0, gan_set.shape[0], number_of_batches):  # Iterate with batch_size_discriminator steps
            ...
            # Update D network: maximize Wasserstein distance
            # Train with real data
            netD.zero_grad()
            errD_real = -torch.mean(netD(real_batch))
            noise = torch.randn(batch_size, seq_len, input_length, device=device)
            fake = netG(noise)
            errD_fake = torch.mean(netD(fake.detach()))

            # Wasserstein distance loss for the discriminator
            errD = errD_real + errD_fake

            # Gradient penalty
            gradient_penalty = calculate_gradient_penalty(netD, real_batch, fake.detach(), device)
            errD_with_penalty = errD + gradient_penalty

            # Backward and optimize
            errD_with_penalty.backward()
            optimizerD.step()

            # Update G network: maximize Wasserstein distance
            ...
            # Compute D(G(z)) for logging
            output = netD(fake)  # Output of the discriminator for the generated fake data
...
# Run loops
file_path = "2 BHP parameters Wasserstein.txt"
...
for j in range(0, tuning_iterations):
    ...
    if min_wasserstein.size > 0:
        ...
        with open("2 BEST W-GAN Tuning Wasserstein Distance BHP.txt", "a") as file:
...
torch.save(best_generator.state_dict(), '2 BHP W-GAN Gen')
