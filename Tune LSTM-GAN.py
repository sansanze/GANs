...
class LSTMDiscriminator(nn.Module):
    ...
    def __init__(self, in_dim, n_layers, hidden_dim):
        super().__init__()
        self.n_layers = n_layers
        self.hidden_dim = hidden_dim
...
dim_dis_dist = normal_dist(0, 0.5, 50, 300)
...
def train_GAN(number_of_batches, input_length, batch_length, step_size, pretrain_epochs, lr_gen, lr_dis, layers_gen,
    ...
    for i in range(num_segments):
        segment = log_returns_train[i * step_size: i * step_size + batch_length].reshape(batch_length, 1)
        gan_set[i, :, :] = segment

    # Convert the NumPy array to a PyTorch tensor
    gan_set = torch.from_numpy(gan_set).float()  # Convert to PyTorch tensor

    # Create a BytesIO buffer to save and load the NumPy array
    ...
    netD = LSTMDiscriminator(in_dim=1, n_layers=layers_dis, hidden_dim=dim_dis).to(device)
...
file_path = "BHP parameters LSTM.txt"
...
for j in range(0, tuning_iterations):
    ...
    for k in range(1, param_adjustment + 1):
        ...
        parameters[k, 10] = dim_dis_dist(parameters[0, 10])
    ...
    if min_wasserstein.size > 0:
        ...
        with open("BEST LSTM GAN Tuning Wasserstein Distance BHP.txt", "a") as file:
...
torch.save(best_generator.state_dict(), 'LSTM Gen BHP')
